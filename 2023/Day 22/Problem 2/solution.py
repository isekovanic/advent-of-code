...
class Cube:
	...
	def __repr__(self):
		return '{}, ix={}, iy={}, iz={}, supported_by={}, supports={}'.format(self.name, self.ix, self.iy, self.iz, [c.name for c in self.supported_by], [c.name for c in self.supports])
...
class Solver(SolverCore):
	def _solve(self, problem_input):
		...
		for pillar in cannot_disintegrate:
			queue = pillar.supports[::]
			fallen = set([pillar])

			while queue:
				current = queue.pop(0)

				if all(current_supporting in fallen for current_supporting in current.supported_by):
					fallen.add(current)
