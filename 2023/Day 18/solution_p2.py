...
class Solver(SolverCore):
	...
	def shoelace_formula(self, points):
		...
		# going to work. Since it works for cartesian coordinates, only 1 half of the trenches
	...
	def _solve(self, problem_input):
		...
		for instruction in problem_input:
			_, _, encoding = instruction.split()
			length = int(encoding[2:-2], 16)
			direction = 'R D L U'.split()[int(encoding[-2])]
