...
class Solver(SolverCore):
	...
	def get_neighbours(self, node, direction, consecutive_nodes, grid):
		...
		for new_dir, (d_i, d_j) in enumerate(zip(dx, dy)):
			if new_dir == direction and consecutive_nodes == 3 or directions[direction] == (-d_i, -d_j) or new_dir == 0: # stop going in this direction
				continue
	...
	def dijkstra(self, grid, start, end):
		...
		# While this isn't so typical when implementing it, it's a huge optimization in this case.
		visited = set([])
