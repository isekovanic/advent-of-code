...
class Solver(SolverCore):
    ...
    def _solve(self, problem_input):
        ...
        broken_lights = [(0, 0), (len(grid) - 1, 0), (0, len(grid[0]) - 1), (len(grid) - 1, len(grid[0]) - 1)]
        for x, y in broken_lights:
            grid[x][y] = '#'
            ...
            for i in range(len(grid)):
                for j in range(len(grid[i])):
                    ...
                    if grid[i][j] == '.' and on_neighbours == 3:
                        new_grid[i][j] = '#'
                    elif grid[i][j] == '#' and on_neighbours != 2 and on_neighbours != 3 and (i, j) not in broken_lights:
                        new_grid[i][j] = '.'
