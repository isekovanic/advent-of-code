...
class Solver(SolverCore):
    def _solve(self, problem_input):
        instructions = [line.strip().split(' ') for line in problem_input]
        final_password = list('fbgdceah')
        
        for start_password in permutations(final_password):
            ...
            for instruction in instructions:
                if 'swap' in instruction:
                    ...
                    direction = 'right'
                    steps = 0
                    if 'step' in instruction or 'steps' in instruction:
            ...
            if password == final_password:
                return ''.join(start_password)
        
        # no such password exists, should not happen
        return None


solver = Solver('efghdabc')
solver.solve()